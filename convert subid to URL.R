...
# NOTE: THE TOP HALF IS IF YOUR HEADERS ARE LIKE COMMON.NAME AND THE BOTTOM HALF IS IF YOUR HEADERS ARE LIKE common_name
...
############################################################################################################################################################
############################################################################################################################################################
# THE SNAKECASE VERSION

#make URL link for Shiny table
#turns ebird subid into a clickable html URL

subidtable <- read.delim("March2022dataset.txt", sep="\t", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))

#keep only unique subids
subidunique <- subidtable[!duplicated(subidtable$sampling_event_identifier), ]
...
# concatenate into eBird URL
subidunique$ChecklistLink <-  with(subidunique, paste0(front, sampling_event_identifier, middle, sampling_event_identifier, back))

#Select columns for export:
#Select all columns and only the rows you want:
export <- subidunique[, c("sampling_event_identifier","ChecklistLink")]

#Then export:
write.csv(export, file= "subids_with_url.csv", row.names=FALSE)
